...
fn main() {
    ...
    // -------------------------------------------------------------------------
    // Demo: write "Hello World!" one character at a time, 500 ms apart,
    ...
    // Centre the text in the 240×320 frame
    let x_start: usize = (display::WIDTH as usize - text.len() * char_w) / 2; // = 72
    let y: u16        = (display::HEIGHT - char_h as u16) / 2;                // = 156

    log::info!("Writing \"{}\" character by character", text);

    for (i, ch) in text.chars().enumerate() {
        let x = (x_start + i * char_w) as u8; // always a multiple of 8
        log::info!("  '{}' at ({}, {})", ch, x, y);
